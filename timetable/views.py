...
ttemplete = '<div style="font-size: 30px;">Расписание занятий на'
ttable = [
...
def lessons(week, ttable):
    if ttable is not None:
        ...
        for lesson in ttable:
            ...
            count += 1
        return HttpResponse(f'<b>{ttemplete} {week}:</b><br><br>{lessons}</div>')
    else:
        return HttpResponse(f'<b>{ttemplete} {week}</b><br><br>Выходной.</div>')
...
def friday(request):
    return HttpResponse(lessons('Пятница', ttable[4]))
